...
def run_dlt_model(
    ...
    posterior_samples_dict = mcmc.get_samples(group_by_chain=True)

    # transform them into xr.Dataset
    n_chains, n_draws, n_steps = posterior_samples_dict["dlt_comp"].shape

    dlt_comp_p50 = jnp.median(posterior_samples_dict["dlt_comp"], axis=(0, 1))

    data_vars = {
        "dlt_comp": (["chain", "draw", "time"], posterior_samples_dict["dlt_comp"]),
        ...
        "yhat": (["chain", "draw", "time"], posterior_samples_dict["yhat"]),
        "resid": (["chain", "draw", "time"], y - posterior_samples_dict["yhat"]),
        "reg_comp": (["chain", "draw", "time"], posterior_samples_dict["reg_comp"]),
        "sigma": (["chain", "draw"], posterior_samples_dict["sigma"]),
        "last_lev": (["chain", "draw"], posterior_samples_dict["last_lev"]),
        "last_slp": (["chain", "draw"], posterior_samples_dict["last_slp"]),
    }
    coords = {
    ...
    if regression_scheme is not None:
        ...
        coef_p50 = jnp.median(posterior_samples_dict["coef"], axis=(0, 1))
        # (n_steps, )
        ...
        data_vars.update(
            {
                "coef": (["chain", "draw", "var_name"], posterior_samples_dict["coef"]),
