...
def dlt_transition_step(carry, inputs, lev_sm: float, slp_sm: float, theta: float):
    ...
    y_t = jnp.where(jnp.isnan(y_t), dlt_comp_t + eps_t, y_t)
...
def dlt_model(
    ...
    if reg_input is not None:
        coefs = []
        neu_reg_comp = neg_reg_comp = pos_reg_comp = jnp.zeros_like(y)
        ...
        if neu_reg_input is not None:
            ...
            neu_coef = numpyro.sample("neu_coef", dist.Normal(loc=neu_coef_loc_prior, scale=neu_coef_scale_prior))
...
def run_dlt_model(
    ...
    """Run NUTS/MCMC on the DLT model and return an inference dataset (``idata``) as an ``xr.Dataset``.
    ...
    mcmc.run(rng_key, lev_sm=lev_sm, slp_sm=slp_sm, theta=theta, y=y, reg_input=reg_input)

    posterior_dict = mcmc.get_samples(group_by_chain=True)

    # transform them into xr.Dataset
    n_chains, n_draws, n_steps = posterior_dict["dlt_comp"].shape

    dlt_comp_p50 = jnp.median(posterior_dict["dlt_comp"], axis=(0, 1))

    data_vars = {
        "dlt_comp": (["chain", "draw", "time"], posterior_dict["dlt_comp"]),
        ...
        "yhat": (["chain", "draw", "time"], posterior_dict["yhat"]),
        "resid": (["chain", "draw", "time"], y - posterior_dict["yhat"]),
        "reg_comp": (["chain", "draw", "time"], posterior_dict["reg_comp"]),
        "sigma": (["chain", "draw"], posterior_dict["sigma"]),
        "last_lev": (["chain", "draw"], posterior_dict["last_lev"]),
        "last_slp": (["chain", "draw"], posterior_dict["last_slp"]),
    }
    coords = {
    ...
    if regression_scheme is not None:
        # add seasonal regressor coefficient samples
        coef_p50 = jnp.median(posterior_dict["coef"], axis=(0, 1))
        # (n_steps, )
        ...
        data_vars.update(
            {
                "coef": (["chain", "draw", "var_name"], posterior_dict["coef"]),
    ...
    idata = xr.Dataset(data_vars=data_vars, coords=coords)
...
def generate_dlt_components(
    ...
    slp_sm: float,
    theta: float,
    oos_y: Optional[jnp.ndarray] = None,
) -> jnp.ndarray:
    ...
    posterior level/slope, with noise sampled from the ``sigma`` posterior driving
    ...
    idata: ``xr.Dataset`` returned by ``run_dlt_model``, must contain
    ...
    """
    if idata.get("dlt_comp") is None:
        raise ValueError("idata must contain 'dlt_comp' variable.")
    else:
        # in-sample forecast
        # (n_samples, n__train_steps)
        dlt_comp_is_samples = flatten_front_dim(idata["dlt_comp"].to_numpy(), n=2)
    ...
    if end_step > n_train_steps:
        ...
        # (n_samples, )
        last_lev = flatten_front_dim(idata["last_lev"].to_numpy(), n=2)
        last_slp = flatten_front_dim(idata["last_slp"].to_numpy(), n=2)
        ...
        eps_samples = generate_noise_components(rng_key, idata, oos_steps).transpose()
        ...
        logger.debug("Concatenating in-sample and out-of-sample forecasts.")
        dlt_comp_oos_samples = jnp.transpose(dlt_comp_oos_samples, axes=(-1, -2))
        dlt_comp_samples = jnp.concatenate((dlt_comp_is_samples, dlt_comp_oos_samples), axis=-1)
    else:
...
def make_inference(
    ...
    When ``covariates_df`` is supplied and ``"coef"`` is present in ``idata``,
    ...
    ----
    rng_key: JAX random key for reproducibility.
    idata: ``xr.Dataset`` returned by ``run_dlt_model``.
    lev_sm: Level smoothing factor.
    slp_sm: Slope smoothing factor.
    theta: Damping factor.
    end_step: Total steps to forecast. Ignored (overridden) when covariates are given;
        required otherwise.
    covariates_df: Optional DataFrame of covariates, shape ``(n_steps, n_var)``.
        Column names must match the ``var_name`` coordinate in ``idata``.
    ...
    """
    if "coef" in idata and covariates_df is not None:
        # (n_samples, n_var)
        coef = flatten_front_dim(idata["coef"].to_numpy(), n=2)
        var_names = list(idata["coef"].var_name.values)
    ...
    dlt_comp_samples = generate_dlt_components(rng_key, idata, end_step, lev_sm, slp_sm, theta)
    logger.debug(f"dlt_comp_samples shape: {dlt_comp_samples.shape}")

    rng_key = jax.random.split(rng_key, num=1)[0]
    eps_samples = generate_noise_components(rng_key, idata, end_step)
