...
def generate_forecast_span_samples(posterior: xr.Dataset, h: int) -> jnp.ndarray:
    # how to do a sliding window prediction?
    dlt_comp = flatten_front_dim(posterior["dlt_comp"].to_numpy(), n=2)
    reg_comp = flatten_front_dim(posterior["reg_comp"].to_numpy(), n=2)
...
def run_dlt_model_and_compute_wbic(params: Tuple, data: Dict[str, np.ndarray], h: int = 12) -> float:
    ...
    yhat_span = generate_forecast_span_samples(posterior=posterior, h=h)
