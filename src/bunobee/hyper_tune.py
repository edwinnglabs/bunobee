...
def generate_forecast_span_samples(posterior_samples: xr.Dataset, h: int) -> jnp.ndarray:
    # how to do a sliding window prediction?
    dlt_comp = flatten_front_dim(posterior_samples["dlt_comp"].to_numpy(), n=2)
    reg_comp = flatten_front_dim(posterior_samples["reg_comp"].to_numpy(), n=2)
...
def run_dlt_model_and_compute_wbic(params: Tuple, data: Dict[str, np.ndarray], h: int = 12) -> float:
    ...
    posterior_samples = run_dlt_model(
        lev_sm=lev_sm,
        slp_sm=slp_sm,
        theta=theta,
        x_seas=x_seas,
    ...
    yhat_span = generate_forecast_span_samples(posterior_samples=posterior_samples, h=h)
